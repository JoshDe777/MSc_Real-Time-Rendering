...
namespace Maze {
    ...
    Torch::Torch(EisEngine::Game &game) {
        entity = ResourceManager::Load3DObject(game, "3d-objects/own_torch.fbx");
        entity->transform->SetLocalScale(Vector3(0.0003f, 0.0003f, 0.0003f));
        entity->transform->SetLocalRotation(Vector3(-90, 90, 0));
        std::vector<Transform*> unexplored = {entity->transform};
        while(flame == nullptr && !unexplored.empty()){
            ...
            if(name == "Flame"){
                flame = current->entity()->GetComponent<Renderer>();
                break;
            }
