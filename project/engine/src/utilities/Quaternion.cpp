...
namespace EisEngine {
    ...
    Vector3 Quaternion::ToEulerXYZ() const {
        auto pitch = Math::Arcsin(-2 * (y*z + r*x), RADIANS);
        auto cosP = Math::Cos(pitch, RADIANS);
        // gimbal lock bypassing
        // case no GL
        float yaw;
        float roll;
        if(cosP != 0){
            yaw = Math::Arctan(2*x*z - 2*r*y, 1.0f-2*pow(x, 2)-2*pow(y,2), RADIANS);
            roll = Math::Arctan(x*y-r*z, 0.5f-pow(x,2)-pow(z,2));
        }
        else{
            yaw = Math::Arctan(-x*z-r*y, 0.5f-pow(y,2)-pow(z,2), RADIANS);
            roll = 0.0f;
        }
    ...
    Quaternion Quaternion::Lerp(const EisEngine::Quaternion &q1, const EisEngine::Quaternion &q2, const float &val) {
        ...
        return Quaternion(
            Math::Lerp(q1.x, q2.x, val),
            Math::Lerp(q1.y, q2.y, val),
            Math::Lerp(q1.z, q2.z, val),
            Math::Lerp(q1.r, q2.r, val)
        ).normalized();
    }
